# Hi, I'm zshs000 👋

CS专业大三学生 | Java 后端开发实习生 | AI 工程化落地实践者   
🔍 核心探索方向：**Harness Engineering** —— 通过构建结构化控制系统，可靠地驾驭AI Agent   
💡 专注于：把想法落地为生产级项目，同时参与开源    
✨ 这里记录：我的学习、项目与开源成长之路  
## 📊 GitHub Stats & Top Languages

<div align="center">
  <img src="https://github-readme-stats-fast.vercel.app/api?username=zshs000&show_icons=true&theme=tokyonight&hide_border=true&hide_rank=true" ... />
  <img src="https://github-readme-stats-fast.vercel.app/api/top-langs/?username=zshs000&layout=compact&theme=onedark&hide_border=true&langs_count=8" width="40%" />
</div>

### 🛠️ 技术栈 & 技能
- **后端**：Java（Spring Boot / Spring Cloud Alibaba）
- **AI / 大模型应用**：Python、RAG、AI Agent 开发、Claude 等大模型工具链、Harness Engineering
- **其他**：Git、Linux 基本操作、Markdown 写作  

### 🔥 最近在做 / 亮点项目
- **[rag-bilibili](https://github.com/zshs000/rag-bilibili)**：对 Spring AI Alibaba 的 **bilibilireader** 组件的实战落地 —— 基于 B 站视频字幕构建的 RAG 检索增强问答系统（支持 BV 号导入、自动切分向量化、单视频/全视频检索、流式问答）
- **[task_scheduler](https://github.com/zshs000/task_scheduler)**：给本地 AI 助手加“闹钟”功能的定时任务工具（支持自然语言、邮件提醒、跨平台）—— Python 项目
- **[ExcelAIQ-Bank](https://github.com/zshs000/ExcelAIQ-Bank)**：基于 Spring Cloud Alibaba 的分布式题库管理系统，[初心文档](https://github.com/zshs000/ExcelAIQ-Bank/blob/master/docs/项目初心.md)（微服务实战 + 真实政务场景痛点落地）
- 第一次真实开源贡献：给 **[Alibaba Spring Cloud Alibaba](https://github.com/alibaba/spring-cloud-alibaba)** 提 PR（修复日志中敏感字段泄露风险），已被合并进 2025.1.x 版本！（超级开心～）  
- CSDN 博客持续输出：Java 底层、AI 应用、开源贡献记录、项目实战反思等文章  

### 📫 怎么找我
- **Email**：2862877643@qq.com（欢迎邮件聊项目、问问题、交流 AI/Java！）  
- **博客**：https://blog.csdn.net/zsy520real （欢迎围观～）  
- GitHub Discussions / Issues：直接评论我的仓库也 OK！

### 🌱 正在学 / 下一步计划
- 深入 **Harness Engineering** 实践（让 AI Agent 可靠地完成复杂任务）
- 继续贡献开源（欢迎推荐有趣的项目！）  
- 完善更多小工具，争取让它们更有用
- 在真实业务场景中打磨工程能力与分布式系统实践

Thanks for stopping by! ✨  
期待和你一起玩转代码 & AI ～ 😄
